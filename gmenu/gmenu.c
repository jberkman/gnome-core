...
Desktop_Data *current_desktop = NULL;
gchar *current_path;
...
GnomeUIInfo sort_menu[] = {
	{ GNOME_APP_UI_ITEM, N_("_Sort Submenu"), NULL, sort_single_pressed, NULL, NULL,
	  GNOME_APP_PIXMAP_STOCK, GNOME_STOCK_MENU_SPELLCHECK, 'S',
	  GDK_CONTROL_MASK, NULL },
	{ GNOME_APP_UI_ITEM, N_("Sort Submenu _Recursive"), NULL, sort_recursive_pressed, NULL, NULL,
...
/* toolbar */
GnomeUIInfo toolbar[] = {
	{ GNOME_APP_UI_ITEM, N_("New Submenu"), N_("Create a new submenu"), create_folder_pressed_cb, NULL, NULL,
	...
	{ GNOME_APP_UI_ITEM, N_("Sort Submenu"), N_("Sort selected submenu"), sort_single_pressed, NULL, NULL,
...
*/
	...
	gmenu_init_dnd(GTK_CTREE(menu_tree_ctree));
	...
	infopixmap = gnome_stock_pixmap_widget_new(app,
...
static void sort_recurse_cb(GtkCTree *ctree, GtkCTreeNode *node, gpointer data)
{
	...
	if (d->isfolder) tree_sort_node (node);
}
