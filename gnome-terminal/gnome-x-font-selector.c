...
#include "gnome-x-font-selector.h"
...
#define INITIAL_METRIC		  GNOME_FONT_METRIC_POINTS
...
/* This represents one style, as displayed in the Font Style clist. It can
   ...
   GnomeFontType in the flags field. */
#define  GNOME_FONT_DISPLAYED	(1 << 7)

struct _FontStyle
{
  guint16  properties[GNOME_NUM_STYLE_PROPERTIES];
...
struct _GtkFontSelInfo {
  ...
  gchar **properties[GNOME_NUM_FONT_PROPERTIES];
  guint16 nproperties[GNOME_NUM_FONT_PROPERTIES];
  guint16 space_allocated[GNOME_NUM_FONT_PROPERTIES];
};
...
/* This is used to look up a field in a fontname given one of the above
   property indices. */
static const FontField xlfd_index[GNOME_NUM_FONT_PROPERTIES] = {
  XLFD_WEIGHT,
  XLFD_SLANT,
  XLFD_SET_WIDTH,
  XLFD_SPACING,
...
static const gint filter_positions[GNOME_NUM_FONT_PROPERTIES][2] = {
  { 1, 0 }, { 0, 2 }, { 1, 2 }, { 2, 2 }, { 2, 0 }, { 0, 0 }
};
static const gint filter_heights[GNOME_NUM_FONT_PROPERTIES] = {
  100, 70, 70, 40, 100, 100
};

/* This is returned by gnome_font_selection_filter_state to describe if a
...
static void    gnome_font_selection_instance_init    (GnomeFontSelection *fontsel);
...
static void    gnome_font_selection_get_fonts          (void);
static void    gnome_font_selection_insert_font        (GSList         *fontnames[],
						      gint           *ntable,
						      gchar          *fontname);
static gint    gnome_font_selection_insert_field       (gchar          *fontname,
						      gint            prop);

/* These are the callbacks & related functions. */
static void    gnome_font_selection_select_font	     (GtkWidget      *w,
						      gint	      row,
						      gint	      column,
						      GdkEventButton *bevent,
						      gpointer        data);
static gint    gnome_font_selection_on_clist_key_press (GtkWidget      *clist,
						      GdkEventKey    *event,
						      GnomeFontSelection *fs);
static gboolean gnome_font_selection_select_next	     (GnomeFontSelection *fs,
						      GtkWidget        *clist,
						      gint		step);
static void    gnome_font_selection_show_available_styles
(GnomeFontSelection *fs);
static void    gnome_font_selection_select_best_style  (GnomeFontSelection *fs,
						      gboolean	       use_first);

static void    gnome_font_selection_select_style	     (GtkWidget      *w,
						      gint	      row,
						      gint	      column,
						      GdkEventButton *bevent,
						      gpointer        data);
static void    gnome_font_selection_show_available_sizes
(GnomeFontSelection *fs);
static void     gnome_font_selection_size_activate  (GtkWidget *w,
						   gpointer   data);

static void    gnome_font_selection_select_best_size   (GnomeFontSelection *fs);
static void    gnome_font_selection_select_size	     (GtkWidget      *w,
						      gint	      row,
						      gint	      column,
						      GdkEventButton *bevent,
						      gpointer        data);

static void    gnome_font_selection_metric_callback    (GtkWidget      *w,
						      gpointer        data);
static void    gnome_font_selection_expose_list	     (GtkWidget	     *w,
						      GdkEventExpose *event,
						      gpointer        data);
static void    gnome_font_selection_realize_list	     (GtkWidget	     *widget,
						      gpointer	      data);

static void    gnome_font_selection_switch_page	     (GtkWidget      *w,
						      GtkNotebookPage *page,
						      gint             page_num,
						      gpointer         data);
static void    gnome_font_selection_show_font_info     (GnomeFontSelection *fs);

static void    gnome_font_selection_select_filter	     (GtkWidget      *w,
						      gint	      row,
						      gint	      column,
						      GdkEventButton *bevent,
						      GnomeFontSelection *fs);
static void    gnome_font_selection_unselect_filter    (GtkWidget      *w,
						      gint	      row,
						      gint	      column,
						      GdkEventButton *bevent,
						      GnomeFontSelection *fs);
static void    gnome_font_selection_update_filter	     (GnomeFontSelection *fs);
static gboolean gnome_font_selection_style_visible     (GnomeFontSelection *fs,
						      FontInfo       *font,
						      gint	      style);
static void    gnome_font_selection_reset_filter	     (GtkWidget      *w,
						      GnomeFontSelection *fs);
static void    gnome_font_selection_on_clear_filter    (GtkWidget      *w,
						      GnomeFontSelection *fs);
static void    gnome_font_selection_show_available_fonts
						     (GnomeFontSelection *fs);
static void    gnome_font_selection_clear_filter       (GnomeFontSelection *fs);
static void    gnome_font_selection_update_filter_lists(GnomeFontSelection *fs);
static GtkFontPropertyFilterState gnome_font_selection_filter_state
						     (GnomeFontSelection *fs,
...
static gboolean gnome_font_selection_load_font         (GnomeFontSelection *fs);
static void    gnome_font_selection_update_preview     (GnomeFontSelection *fs);

static gint    gnome_font_selection_find_font	     (GnomeFontSelection *fs,
						      gchar          *family,
						      guint16	      foundry);
static guint16 gnome_font_selection_field_to_index     (gchar         **table,
						      gint            ntable,
						      gchar          *field);

static gchar*  gnome_font_selection_expand_slant_code  (gchar          *slant);
static gchar*  gnome_font_selection_expand_spacing_code(gchar          *spacing);

/* Functions for handling X Logical Font Description fontnames. */
static gboolean gnome_font_selection_is_xlfd_font_name (const gchar    *fontname);
static char*   gnome_font_selection_get_xlfd_field     (const gchar    *fontname,
						      FontField       field_num,
						      gchar          *buffer);
static gchar * gnome_font_selection_create_xlfd        (gint            size,
...
static void    gnome_font_selector_instance_init (GnomeFontSelector *fontseldiag);

static gint    gnome_font_selector_on_configure(GtkWidget      *widget,
						      GdkEventConfigure *event,
						      GnomeFontSelector *fsd);
...
gnome_font_selection_get_type()
{
  ...
  if(!font_selection_type)
	...
	(GtkObjectInitFunc) gnome_font_selection_instance_init,
	/* reserved_1 */ NULL,
	/* reserved_2 */ NULL,
...
gnome_font_selection_class_init(GnomeFontSelectionClass *klass)
{
  ...
  gnome_font_selection_get_fonts ();
}

static void
gnome_font_selection_instance_init(GnomeFontSelection *fontsel)
{
  ...
  /* Initialize the GnomeFontSelection struct. We do this here in case any
  ...
  fontsel->filters[GNOME_FONT_FILTER_BASE].font_type = GNOME_FONT_ALL;
  fontsel->filters[GNOME_FONT_FILTER_USER].font_type = GNOME_FONT_BITMAP
    | GNOME_FONT_SCALABLE;

  
  for (prop = 0; prop < GNOME_NUM_FONT_PROPERTIES; prop++)
    {
      fontsel->filters[GNOME_FONT_FILTER_BASE].property_filters[prop] = NULL;
      fontsel->filters[GNOME_FONT_FILTER_BASE].property_nfilters[prop] = 0;
      fontsel->filters[GNOME_FONT_FILTER_USER].property_filters[prop] = NULL;
      fontsel->filters[GNOME_FONT_FILTER_USER].property_nfilters[prop] = 0;
    }
  
  for (prop = 0; prop < GNOME_NUM_STYLE_PROPERTIES; prop++)
  ...
  gtk_signal_connect (GTK_OBJECT (fontsel->size_entry), "activate",
		      GTK_SIGNAL_FUNC (gnome_font_selection_size_activate),
		      fontsel);
  ...
  gtk_signal_connect (GTK_OBJECT (fontsel->size_clist), "select_row",
		      GTK_SIGNAL_FUNC(gnome_font_selection_select_size),
		      fontsel);
  ...
  gtk_signal_connect (GTK_OBJECT (fontsel->filter_button), "clicked",
		      GTK_SIGNAL_FUNC(gnome_font_selection_on_clear_filter),
		      fontsel);
  ...
  if (INITIAL_METRIC == GNOME_FONT_METRIC_PIXELS)
  ...
  gtk_signal_connect(GTK_OBJECT(fontsel->points_button), "toggled",
		     (GtkSignalFunc) gnome_font_selection_metric_callback,
		     fontsel);
...
#ifdef HAVE_A_NON_PANGO_GTK_ENTRY
  ...
  gtk_signal_connect (GTK_OBJECT (fontsel), "switch_page",
		      GTK_SIGNAL_FUNC(gnome_font_selection_switch_page),
		      fontsel);
  ...
  for (prop = 0; prop < GNOME_NUM_FONT_PROPERTIES; prop++)
      ...
      if (top == 2 && left == 2)
	{
	  ...
	  gtk_signal_connect (GTK_OBJECT (button), "clicked",
			      GTK_SIGNAL_FUNC(gnome_font_selection_reset_filter),
			      fontsel);
	}
      else
	gtk_table_attach (GTK_TABLE (table), scrolled_win,
      ...
      gtk_signal_connect (GTK_OBJECT (clist), "select_row",
			  GTK_SIGNAL_FUNC(gnome_font_selection_select_filter),
			  fontsel);
      gtk_signal_connect (GTK_OBJECT (clist), "unselect_row",
			  GTK_SIGNAL_FUNC(gnome_font_selection_unselect_filter),
			  fontsel);
	...
	if (prop == SLANT)
	  property = gnome_font_selection_expand_slant_code(property);
	else if (prop == SPACING)
	  property = gnome_font_selection_expand_spacing_code(property);
...
/* This is called when the clist is exposed. Here we scroll to the current
   font if necessary. */
static void
gnome_font_selection_expose_list (GtkWidget		*widget,
				GdkEventExpose		*event,
				gpointer		 data)
{
  GnomeFontSelection *fontsel;
...
static void
gnome_font_selection_realize_list (GtkWidget		*widget,
				 gpointer		 data)
{
  GnomeFontSelection *fontsel;
  gint row;
...
static void
gnome_font_selection_select_font (GtkWidget      *w,
				gint	        row,
				gint	        column,
				GdkEventButton *bevent,
				gpointer        data)
{
  ...
  gnome_font_selection_show_available_styles (fontsel);
  gnome_font_selection_select_best_style (fontsel, TRUE);
}


static gint
gnome_font_selection_on_clist_key_press (GtkWidget        *clist,
				       GdkEventKey      *event,
				       GnomeFontSelection *fontsel)
{
#ifdef FONTSEL_DEBUG
  ...
  if (event->keyval == GDK_Up)
    return gnome_font_selection_select_next (fontsel, clist, -1);
...
static gboolean
gnome_font_selection_select_next (GnomeFontSelection *fontsel,
				GtkWidget        *clist,
				gint		  step)
{
  GList *selection;
...
static void
gnome_font_selection_show_available_styles (GnomeFontSelection *fontsel)
{
  FontInfo *font;
  FontStyle *styles;
  ...
  for (style = 0; style < font->nstyles; style++)
    {
      if (gnome_font_selection_style_visible(fontsel, font, style))
	{
	  styles[style].flags &= ~GNOME_FONT_DISPLAYED;
  ...
  for (style = 0; style < font->nstyles; style++)
    {
      if (styles[style].flags & GNOME_FONT_DISPLAYED)
	continue;
	...
	{
	  if (styles[tmpstyle].flags & GNOME_FONT_DISPLAYED
	      || charset_index != styles[tmpstyle].properties[CHARSET])
	    continue;
	  
	  styles[tmpstyle].flags |= GNOME_FONT_DISPLAYED;
...
static void
gnome_font_selection_select_best_style(GnomeFontSelection *fontsel,
				     gboolean	       use_first)
{
  FontInfo *font;
  FontStyle *styles;
  ...
  if (gtk_clist_row_is_visible(GTK_CLIST(fontsel->font_style_clist), best_row)
    ...
    gtk_clist_moveto(GTK_CLIST(fontsel->font_style_clist), best_row, -1,
		     0.5, 0);
  gnome_font_selection_show_available_sizes (fontsel);
  gnome_font_selection_select_best_size (fontsel);
}


/* This is called when a style is selected in the list. */
static void
gnome_font_selection_select_style (GtkWidget      *w,
				 gint	        row,
				 gint	        column,
				 GdkEventButton *bevent,
				 gpointer        data)
...
static void
gnome_font_selection_show_available_sizes (GnomeFontSelection *fontsel)
{
  FontInfo *font;
  ...
  if (fontsel->metric == GNOME_FONT_METRIC_POINTS)
    {
  ...
  type_filter = fontsel->filters[GNOME_FONT_FILTER_BASE].font_type
    & fontsel->filters[GNOME_FONT_FILTER_USER].font_type;

  if (!((style->flags & GNOME_FONT_SCALABLE_BITMAP
	 && type_filter & GNOME_FONT_SCALABLE_BITMAP)
	|| (style->flags & GNOME_FONT_SCALABLE
	    && type_filter & GNOME_FONT_SCALABLE)))
    nstandard_sizes = 0;
...
static void
gnome_font_selection_update_size (GnomeFontSelection *fontsel)
{
  gint new_size;
  ...
  if (fontsel->metric == GNOME_FONT_METRIC_PIXELS)
    {
      new_size = atoi (text);
      if (new_size < 2)
	new_size = 2;
...
/* If the user hits return in the font size entry, we change to the new font
   size. */
static void
gnome_font_selection_size_activate (GtkWidget *w,
				  gpointer   data)
{
  gnome_font_selection_update_size (data);
}
...
/* This tries to select the closest size to the current size, though it
   ...
   Note: this will load a font. */
static void
gnome_font_selection_select_best_size(GnomeFontSelection *fontsel)
{
  FontInfo *font;
  FontStyle *styles, *style;
  gchar *text;
  ...
  gnome_font_selection_load_font (fontsel);
}


/* This is called when a size is selected in the list. */
static void
gnome_font_selection_select_size (GtkWidget      *w,
				gint	        row,
				gint	        column,
				GdkEventButton *bevent,
				gpointer        data)
...
static void
gnome_font_selection_metric_callback (GtkWidget *w,
				    gpointer   data)
{
  ...
  if (GTK_TOGGLE_BUTTON(fontsel->pixels_button)->active)
    {
      if (fontsel->metric == GNOME_FONT_METRIC_PIXELS)
	return;
      fontsel->metric = GNOME_FONT_METRIC_PIXELS;
  ...
  else
    {
      if (fontsel->metric == GNOME_FONT_METRIC_POINTS)
	return;
      fontsel->metric = GNOME_FONT_METRIC_POINTS;
      fontsel->size *= 10;
...
static guint16
gnome_font_selection_field_to_index (gchar **table,
				   gint    ntable,
				   gchar  *field)
{
  gint i;
...
static gboolean
gnome_font_selection_load_font (GnomeFontSelection *fontsel)
{
  GdkFont *font;
  ...
  fontname = gnome_font_selection_get_font_name (fontsel);
  if (fontname)
    {
#ifdef FONTSEL_DEBUG
      g_message("Loading: %s\n", fontname);
...
static void
gnome_font_selection_switch_page (GtkWidget       *w,
				GtkNotebookPage *page,
				gint             page_num,
				gpointer         data)
{
  ...
  if (page_num == 0)
    gnome_font_selection_update_filter(fontsel);
  else if (page_num == 1)
    gnome_font_selection_show_font_info(fontsel);
}
...
gnome_font_selection_show_font_info (GnomeFontSelection *fontsel)
{
  ...
  for (i = 0; i < GTK_XLFD_NUM_FIELDS; i++)
    {
      if (fontname)
	field = gnome_font_selection_get_xlfd_field (fontname, i, field_buffer);
      else
	field = NULL;
      if (field)
	{
	  if (i == XLFD_SLANT)
	    field = gnome_font_selection_expand_slant_code(field);
	  else if (i == XLFD_SPACING)
	    field = gnome_font_selection_expand_spacing_code(field);
	}
  ...
  if (fontsel->font)
      ...
      if (status == True)
	{
	  ...
	  for (i = 0; i < GTK_XLFD_NUM_FIELDS; i++)
	    {
	      field = gnome_font_selection_get_xlfd_field (name, i,
							 field_buffer);
	      if (i == XLFD_SLANT)
		field = gnome_font_selection_expand_slant_code(field);
	      else if (i == XLFD_SPACING)
...
static void
gnome_font_selection_select_filter	     (GtkWidget      *w,
					      gint	      row,
					      gint	      column,
					      GdkEventButton *bevent,
					      GnomeFontSelection *fontsel)
{
  ...
  if (row == 0)
	...
	if (fontsel->filter_clists[prop] == w)
      ...
      if (gnome_font_selection_filter_state (fontsel, GNOME_FONT_FILTER_BASE,
...
static void
gnome_font_selection_unselect_filter	     (GtkWidget      *w,
					      gint	      row,
					      gint	      column,
					      GdkEventButton *bevent,
					      GnomeFontSelection *fontsel)
	  ...
	  for (i = 1; i < GTK_CLIST(w)->rows; i++)
	    {
	      index = GPOINTER_TO_INT (gtk_clist_get_row_data(GTK_CLIST(w),
							      i));
	      if (gnome_font_selection_filter_state (fontsel,
...
/* This is called when the main notebook page is selected. It checks if the
   ...
   turned off. */
static void
gnome_font_selection_update_filter     (GnomeFontSelection *fontsel)
{
  GtkWidget *clist;
  GList *selection;
  gboolean default_filter = TRUE, filter_changed = FALSE;
  gint prop, nselected, i, row, index;
  GnomeFontFilter *filter = &fontsel->filters[GNOME_FONT_FILTER_USER];
  ...
  base_font_type = fontsel->filters[GNOME_FONT_FILTER_BASE].font_type
    & GNOME_FONT_ALL;
  user_font_type = fontsel->filters[GNOME_FONT_FILTER_USER].font_type
    & GNOME_FONT_ALL;
  ...
  new_font_type |= (GTK_TOGGLE_BUTTON(fontsel->type_scalable_button)->active
    ? GNOME_FONT_SCALABLE : 0);
  new_font_type |= (GTK_TOGGLE_BUTTON(fontsel->type_scaled_bitmaps_button)->active ? GNOME_FONT_SCALABLE_BITMAP : 0);
  new_font_type &= base_font_type;
  new_font_type |= (~base_font_type & user_font_type);
  if (new_font_type != (GNOME_FONT_BITMAP | GNOME_FONT_SCALABLE))
    default_filter = FALSE;

  if (new_font_type != user_font_type)
    filter_changed = TRUE;
  fontsel->filters[GNOME_FONT_FILTER_USER].font_type = new_font_type;
...
static void
gnome_font_selection_show_available_fonts     (GnomeFontSelection *fontsel)
{
  FontInfo *font_info, *font;
  GnomeFontFilter *filter;
  ...
  for (i = 0; i < nfonts; i++)
    {
      ...
      matched = TRUE;
      for (k = 0; k < GNOME_NUM_FONT_FILTERS; k++)
	{
  ...
  gnome_font_selection_show_available_styles (fontsel);
  gnome_font_selection_select_best_style (fontsel, FALSE);
}


/* Returns TRUE if the style is not currently filtered out. */
static gboolean
gnome_font_selection_style_visible(GnomeFontSelection *fontsel,
				 FontInfo         *font,
				 gint              style_index)
{
  FontStyle *styles, *style;
  ...
  for (prop = 0; prop < GNOME_NUM_STYLE_PROPERTIES; prop++)
    {
      ...
      /* Check each filter. */
      for (i = 0; i < GNOME_NUM_FONT_FILTERS; i++)
	{
...
static void
gnome_font_selection_reset_filter	     (GtkWidget      *w,
				      GnomeFontSelection *fontsel)
{
  ...
  base_font_type = fontsel->filters[GNOME_FONT_FILTER_BASE].font_type;
  if (base_font_type & GNOME_FONT_BITMAP)
    gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (fontsel->type_bitmaps_button), TRUE);
  if (base_font_type & GNOME_FONT_SCALABLE)
    gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (fontsel->type_scalable_button), TRUE);
  if (base_font_type & GNOME_FONT_SCALABLE_BITMAP)
...
static void
gnome_font_selection_on_clear_filter     (GtkWidget      *w,
					GnomeFontSelection *fontsel)
{
  gnome_font_selection_clear_filter(fontsel);
}
...
static void
gnome_font_selection_clear_filter     (GnomeFontSelection *fontsel)
{
  GnomeFontFilter *filter;
  gint prop;
  ...
  filter = &fontsel->filters[GNOME_FONT_FILTER_USER];
  filter->font_type = GNOME_FONT_BITMAP | GNOME_FONT_SCALABLE;
  for (prop = 0; prop < GNOME_NUM_FONT_PROPERTIES; prop++)
    {
      g_free(filter->property_filters[prop]);
      filter->property_filters[prop] = NULL;
  ...
  gnome_font_selection_reset_filter(NULL, fontsel);
...
void
gnome_font_selection_set_filter	(GnomeFontSelection *fontsel,
				 GnomeFontFilterType filter_type,
...
{
  GnomeFontFilter *filter;
  gchar **filter_strings [GNOME_NUM_FONT_PROPERTIES];
  gchar *filter_string;
      ...
      if (filter_strings[prop])
	{
	  ...
	  for (i = 0; i < nfilters; i++)
	      ...
	      for (j = 0; j < fontsel_info->nproperties[prop]; j++)
		{
		  ...
		  property_alt = NULL;
		  if (prop == SLANT)
		    property_alt = gnome_font_selection_expand_slant_code(property);
		  else if (prop == SPACING)
		    property_alt = gnome_font_selection_expand_spacing_code(property);
  ...
  gnome_font_selection_update_filter_lists (fontsel);

  if (filter_type == GNOME_FONT_FILTER_BASE)
    {
      user_font_type = fontsel->filters[GNOME_FONT_FILTER_USER].font_type;
      if (font_type & GNOME_FONT_BITMAP)
	{
	  gtk_widget_set_sensitive (fontsel->type_bitmaps_button, TRUE);
	  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (fontsel->type_bitmaps_button), user_font_type & GNOME_FONT_BITMAP);
	}
      else
	{
      ...
      if (font_type & GNOME_FONT_SCALABLE)
	{
	  gtk_widget_set_sensitive (fontsel->type_scalable_button, TRUE);
	  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (fontsel->type_scalable_button), user_font_type & GNOME_FONT_SCALABLE);
	}
      else
	{
      ...
      if (font_type & GNOME_FONT_SCALABLE_BITMAP)
	{
	  gtk_widget_set_sensitive (fontsel->type_scaled_bitmaps_button, TRUE);
	  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (fontsel->type_scaled_bitmaps_button), user_font_type & GNOME_FONT_SCALABLE_BITMAP);
	}
      else
	{
  ...
  else
    {
      base_font_type = fontsel->filters[GNOME_FONT_FILTER_BASE].font_type;
      if (base_font_type & GNOME_FONT_BITMAP)
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (fontsel->type_bitmaps_button), font_type & GNOME_FONT_BITMAP);

      if (base_font_type & GNOME_FONT_SCALABLE)
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (fontsel->type_scalable_button), font_type & GNOME_FONT_SCALABLE);

      if (base_font_type & GNOME_FONT_SCALABLE_BITMAP)
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (fontsel->type_scaled_bitmaps_button), font_type & GNOME_FONT_SCALABLE_BITMAP);

      /* If the user filter is not the default, make the 'Reset Filter' button
	 sensitive. */
      filter_set = FALSE;
      if (font_type != (GNOME_FONT_BITMAP | GNOME_FONT_SCALABLE))
	filter_set = TRUE;
      for (prop = 0; prop < GNOME_NUM_FONT_PROPERTIES; prop++)
	{
	  if (filter->property_nfilters[prop] != 0)
...
static void
gnome_font_selection_update_filter_lists (GnomeFontSelection *fontsel)
{
  GtkWidget *clist;
  ...
  for (prop = 0; prop < GNOME_NUM_FONT_PROPERTIES; prop++)
      ...
      for (row = 1; row < GTK_CLIST(clist)->rows; row++)
	{
	  ...
	  if (gnome_font_selection_filter_state (fontsel, GNOME_FONT_FILTER_USER,
					       prop, index) == FILTERED
	      && fg == NULL)
	    gtk_clist_select_row (GTK_CLIST (clist), row, 0);
	  else
...
gnome_font_selection_filter_state (GnomeFontSelection *fontsel,
				 GnomeFontFilterType filter_type,
				 gint		  property,
				 gint		  index)
{
 ...
 *****************************************************************************/
static void
gnome_font_selection_get_fonts (void)
{
  gchar **xfontnames;
  GSList **fontnames;
  gchar *fontname;
  ...
  for (i = 0; i < num_fonts; i++)
    {
      ...
      g_message("%s\n", xfontnames[i]);
#endif
      if (gnome_font_selection_is_xlfd_font_name (xfontnames[i]))
	gnome_font_selection_insert_font (fontnames, &fontsel_info->nfonts, xfontnames[i]);
      else
	{
#ifdef FONTSEL_DEBUG
  ...
  for (i = 0; i < fontsel_info->nfonts; i++)
      ...
      while (temp_list)
	{
	  ...
	  for (prop = 0; prop < GNOME_NUM_STYLE_PROPERTIES; prop++)
		...
		= gnome_font_selection_insert_field (fontname, prop);
	    }
	  ...
	  field = gnome_font_selection_get_xlfd_field (fontname, XLFD_PIXELS,
						     field_buffer);
	  pixels = atoi(field);
	  
	  field = gnome_font_selection_get_xlfd_field (fontname, XLFD_POINTS,
						     field_buffer);
	  points = atoi(field);
	  
	  field = gnome_font_selection_get_xlfd_field (fontname,
						     XLFD_RESOLUTION_X,
						     field_buffer);
	  res_x = atoi(field);
	  ...
	  if (pixels == 0 && points == 0)
	    {
	      if (res_x == 0 && res_y == 0)
		flags = GNOME_FONT_SCALABLE;
	      else
		flags = GNOME_FONT_SCALABLE_BITMAP;
	    }
	  else
	    flags = GNOME_FONT_BITMAP;
  ...
  for (i = 0; i < fontsel_info->nfonts; i++)
      ...
      for (style = 0; style < font->nstyles; style++)
	{
	  ...
	  if (styles->flags & GNOME_FONT_BITMAP)
	    g_message("Bitmapped font  ");
	  if (styles->flags & GNOME_FONT_SCALABLE)
	    g_message("Scalable font  ");
	  if (styles->flags & GNOME_FONT_SCALABLE_BITMAP)
...
static void
gnome_font_selection_insert_font (GSList		      *fontnames[],
				gint		      *ntable,
				gchar		      *fontname)
{
  FontInfo *table;
  ...
  family = gnome_font_selection_get_xlfd_field (fontname, XLFD_FAMILY,
					      family_buffer);
  if (!family)
    return;
  
  foundry = gnome_font_selection_insert_field (fontname, FOUNDRY);
...
static gint
gnome_font_selection_insert_field (gchar		       *fontname,
				 gint			prop)
{
  gchar field_buffer[XLFD_MAX_FIELD_LEN];
  gchar *field;
  ...
  field = gnome_font_selection_get_xlfd_field (fontname, xlfd_index[prop],
					     field_buffer);
  if (!field)
    return 0;
...
gnome_font_selection_get_font (GnomeFontSelection *fontsel)
{
  ...
  gnome_font_selection_update_size (fontsel);
...
gchar *
gnome_font_selection_get_font_name (GnomeFontSelection *fontsel)
{
  FontInfo *font;
  gchar *family_str, *foundry_str;
  gchar *property_str[GNOME_NUM_STYLE_PROPERTIES];
  gint prop;
  ...
  return gnome_font_selection_create_xlfd (fontsel->size,
					 fontsel->metric,
					 foundry_str,
					 family_str,
					 property_str[WEIGHT],
...
gboolean
gnome_font_selection_set_font_name (GnomeFontSelection *fontsel,
				    const gchar      *fontname)
{
  ...
  g_return_val_if_fail (GNOME_IS_FONT_SELECTION (fontsel), FALSE);
  g_return_val_if_fail (fontname != NULL, FALSE);
  
  /* Check it is a valid fontname. */
  if (!gnome_font_selection_is_xlfd_font_name(fontname))
    return FALSE;
  ...
  field = gnome_font_selection_get_xlfd_field (fontname, XLFD_FOUNDRY,
					     field_buffer);
  foundry = gnome_font_selection_field_to_index (fontsel_info->properties[FOUNDRY],
					       fontsel_info->nproperties[FOUNDRY],
					       field);
  
  index = gnome_font_selection_find_font(fontsel, family, foundry);
  if (index == -1)
    return FALSE;
  ...
  for (prop = 0; prop < GNOME_NUM_STYLE_PROPERTIES; prop++) 
    {
      field = gnome_font_selection_get_xlfd_field (fontname, xlfd_index[prop],
						 field_buffer);
      value = gnome_font_selection_field_to_index (fontsel_info->properties[prop],
						 fontsel_info->nproperties[prop],
						 field);
...
static gint
gnome_font_selection_find_font (GnomeFontSelection *fontsel,
			      gchar	       *family,
			      guint16		foundry)
{
  FontInfo *font_info;
...
/* This returns the text in the preview entry. You should copy the returned
   text if you need it. */
const gchar*
gnome_font_selection_get_preview_text  (GnomeFontSelection *fontsel)
{
...
void
gnome_font_selection_set_preview_text  (GnomeFontSelection *fontsel,
					const gchar	  *text)
{
 ...
 */
static gboolean
gnome_font_selection_is_xlfd_font_name (const gchar *fontname)
{
  gint i = 0;
  gint field_len = 0;
 ...
 */
static gchar*
gnome_font_selection_get_xlfd_field (const gchar *fontname,
				   FontField    field_num,
				   gchar       *buffer)
{
  const gchar *t1, *t2;
 ...
 */
static gchar *
gnome_font_selection_create_xlfd (gint		  size,
				GnomeFontMetricType metric,
				gchar		  *foundry,
				gchar		  *family,
				gchar		  *weight,
				...
				gchar		  *charset)
{
  ...
  if (metric == GNOME_FONT_METRIC_PIXELS)
    pixel_size = buffer;
  else
    point_size = buffer;
...
gnome_font_selector_get_type (void)
{
  ...
  if (!font_selection_dialog_type)
	...
	(GtkObjectInitFunc) gnome_font_selector_instance_init,
	/* reserved_1 */ NULL,
	/* reserved_2 */ NULL,
        (GtkClassInitFunc) NULL,
      };
      
      font_selection_dialog_type = gtk_type_unique (
	      GTK_TYPE_DIALOG, &fontsel_diag_info);
    }
...
static void
gnome_font_selector_instance_init (GnomeFontSelector *fontseldiag)
{
  ...
  gtk_signal_connect (GTK_OBJECT (fontseldiag), "configure_event",
		      (GtkSignalFunc) gnome_font_selector_on_configure,
		      fontseldiag);
  
  gtk_container_set_border_width (GTK_CONTAINER (fontseldiag), 4);
  gtk_window_set_policy(GTK_WINDOW(fontseldiag), FALSE, TRUE, TRUE);
  
  fontseldiag->main_vbox = GTK_DIALOG (fontseldiag)->vbox;
  gtk_box_set_spacing (GTK_BOX (fontseldiag->main_vbox), 4);
...
GtkWidget*
gnome_font_selector_new	(const gchar *title)
{
  GnomeFontSelector *fontseldiag;
  
  fontseldiag = gtk_type_new (GNOME_TYPE_FONT_SELECTOR);
...
gchar*
gnome_font_selector_get_font_name	(GnomeFontSelector *fsd)
{
  return gnome_font_selection_get_font_name(GNOME_FONT_SELECTION(fsd->fontsel));
}

GdkFont*
gnome_font_selector_get_font	(GnomeFontSelector *fsd)
{
  return gnome_font_selection_get_font(GNOME_FONT_SELECTION(fsd->fontsel));
}

gboolean
gnome_font_selector_set_font_name	(GnomeFontSelector *fsd,
					 const gchar	  *fontname)
{
  return gnome_font_selection_set_font_name(GNOME_FONT_SELECTION(fsd->fontsel),
					  fontname);
}

void
gnome_font_selector_set_filter	(GnomeFontSelector *fsd,
				 GnomeFontFilterType filter_type,
				 GnomeFontType	   font_type,
				 gchar		 **foundries,
				 gchar		 **weights,
				 ...
				 gchar		 **charsets)
{
  gnome_font_selection_set_filter (GNOME_FONT_SELECTION (fsd->fontsel),
...
const gchar*
gnome_font_selector_get_preview_text (GnomeFontSelector *fsd)
{
  return gnome_font_selection_get_preview_text(GNOME_FONT_SELECTION(fsd->fontsel));
}

void
gnome_font_selector_set_preview_text (GnomeFontSelector *fsd,
					    const gchar	  *text)
{
  gnome_font_selection_set_preview_text(GNOME_FONT_SELECTION(fsd->fontsel), text);
}
...
/* This turns auto-shrink off if the user resizes the width of the dialog.
   ...
   width. */
static gint
gnome_font_selector_on_configure (GtkWidget         *widget,
				  GdkEventConfigure *event,
				  GnomeFontSelector *fsd)
{
 ...
 */
gchar *
gnome_font_select_with_default (const gchar *default_font)
{
	gchar *retval;
	int    response;
	GnomeFontSelector *font_sel;

	font_sel = GNOME_FONT_SELECTOR (gnome_font_selector_new (_("Select a font")));

	/* Create the action area */
	gtk_dialog_add_buttons (GTK_DIALOG (font_sel),
				GTK_STOCK_OK, GTK_RESPONSE_OK,
				GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
				NULL);

	if (default_font)
		gnome_font_selection_set_font_name (
			GNOME_FONT_SELECTION (font_sel->fontsel), default_font);

	response = gtk_dialog_run (GTK_DIALOG (font_sel));

	switch (response) {
	case GTK_RESPONSE_OK:
		retval = gnome_font_selector_get_font_name (font_sel);
		break;
	default:
		retval = NULL;
		break;
...
{
	return gnome_font_select_with_default (NULL);
}

gchar *
gnome_font_selector_get_selected (GnomeFontSelector *text_tool)
{
	return gnome_font_selector_get_font_name (text_tool);
}
